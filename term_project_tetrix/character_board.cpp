...
//public slot: readSourceChars 用于建立字库 当按下选择模式按钮的时候这个函数会被called
void CharacterBoard::buildDictionary(){
    ...
    while (!instream.atEnd()){
         ...
         //这里要根据传入的信号决定读入哪一个dictionary
         if ((mode == 2) && (category == "人文" || category == "基本")){   //也就是说在知道传入的信号之后我会修改这个判断条件，先这样写
 ...
 */
//调整其他块的位置，并检验能不能进一步消去
void CharacterBoard::adjustment(){
    cancle_sound = new QMediaPlayer;
    cancle_sound->setMedia(QMediaContent(QUrl::fromLocalFile("/Users/endlessio/Desktop/cancle.mp3")));
    if (waitingForAdjust){
        ...
        for (int i = 0; i < boardHeight; i++){
            for (int j = 0; j < boardWidth; j++){
                ...
                if (s != ""){
                    ...
                    paintPiece(painter,pieceRect,s);
                }
            }
        }
        if (current !=""){
            ...
            QString cur = current;
            paintPiece(painter,pieceRect,cur);
 ...
 */
//绘制一个块
void CharacterBoard::paintPiece(QPainter &painter,QRect& rect,QString comp){
...
/* Implementation notes: droppedPiece
 * -----------------------------------
 * This function deals with the piece which has
 * dropped to the bottom. It controls the level
 * of the player. It sets the timer to time out
 * so that the program can then check for combination
 */
//处理已经不能再往下掉的方块
 ...
 * -----------------------------------------------
 * These two functions overrides the sizeHint function and the minimumSizeHint
 ...
 * ---------------------------
 * First, it checkes whether the isPause signal is on. If so, it will directly
 * returns. If not, the isStart signal will turn on and initialize all parameters
 ...
 * create a new piece and starts the timer.
 */
 ...
 * ---------------------------
 * First, it checkes whether the isStart signal is on. If not, it will directly
 * returns. Otherwise, the isPause signal will convert itself. After that, if it
 * was false originally, the timer will be stopped. If it was true, the game will
 * start agian.
 */
 ...
 * -----------------------------------
 * First, it checkes whether there is any piece on the board. If not, it will
 * directly returns. Otherwise, a switch statement will be used to detect which
 * key the user presses and calls the corresponding function.
 */
 ...
 * ----------------------------------
 * This functions first tries to find out the largest height it is
 * allowed to move then calls the pieceDrop function to move down the
 * piece.
 */
 ...
 * -----------------------------
 * First, it calculates the position of the possible new piece. Then it checks
 * whether the position is out of the board or whether the position has been
 * occupied. If not, the new piece will replace the current piece and returns true.
 */
...
/* Implementation notes: removeCombination
 * ----------------------------------------
 * This function checks whether the specified locations
 * can be combined with the characters around. These locations
 * shall be checked in 7 ways (correspond to the 7 structures)
 * we defined in the character part
 */
//检查可不可以消去，如果可以则消去.
void CharacterBoard::removeCombination(){
    combine_sound = new QMediaPlayer;
    combine_sound->setMedia(QMediaContent(QUrl::fromLocalFile("/Users/endlessio/Desktop/combine.mp3")));
